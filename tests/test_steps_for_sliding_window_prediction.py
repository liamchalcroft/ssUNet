...
class TestSlidingWindow(unittest2.TestCase):
    ...
    def _verify_steps(self, steps, patch_size, image_size, step_size):
        debug_information = (
            "steps= %s\nimage_size= %s\npatch_size= %s\nstep_size= %0.4f"
            % (str(steps), str(image_size), str(patch_size), step_size)
        )
        ...
        self.assertTrue(
            all([len(i) == num_steps[j] for j, i in enumerate(steps)]),
            "steps do not match expected num_steps %s. \nDebug: %s"
            % (str(num_steps), debug_information),
        )
        ...
        for dim in range(len(steps)):
            ...
            self.assertTrue(
                steps[dim][-1] + patch_size[dim] == image_size[dim],
                "not the whole image is covered. " "\nDebug: %s" % debug_information,
            )
            ...
            self.assertTrue(
                all(
                    [
                        steps[dim][i + 1] <= steps[dim][i] + patch_size[dim]
                        for i in range(num_steps[dim] - 1)
                    ]
                ),
                "steps are not overlapping or touching. dim: %d, steps:"
            ...
            self.assertTrue(
                all(
                    [
                        steps[dim][i] + np.ceil(target_step_sizes_in_voxels[dim])
                        >= steps[dim][i + 1]
                        for i in range(num_steps[dim] - 1)
                    ]
                ),
    ...
    def test_same_image_and_patch_size_3d(self):
        ...
        steps = SegmentationNetwork._compute_steps_for_sliding_window(
            patch_size, image_size, step_size
        )
    ...
    def test_some_manually_verified_combinations(self):
        ...
        self.assertTrue(steps == [[0, 30], [0, 58, 116], [0, 58, 116]])
    ...
    def test_loads_of_combinations(self):
        ...
        for _ in range(5000):
            ...
            image_size = tuple(
                max(image_size[i], patch_size[i]) for i in range(len(image_size))
            )
