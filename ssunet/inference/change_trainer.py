...
def pretend_to_be_nnUNetTrainer(
    folder, checkpoints=("model_best.model.pkl", "model_final_checkpoint.model.pkl")
):
    pretend_to_be_other_trainer(folder, "ContrastivePreTrainer", checkpoints)


def pretend_to_be_other_trainer(
    folder,
    new_trainer_name,
    checkpoints=("model_best.model.pkl", "model_final_checkpoint.model.pkl"),
):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Use this script to change the nnunet trainer class of a saved "
    ...
    parser.add_argument(
        "-i",
        help="Folder containing the trained model. This folder is the one containing the "
        "fold_X subfolders.",
    )
