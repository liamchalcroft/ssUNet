...
def get_augs(
    ...
    tr_transforms.append(
        BrightnessMultiplicativeTransform(
            multiplier_range=(0.70, 1.3), p_per_sample=0.15, data_key=target_key
        )
    )
    tr_transforms.append(
        ContrastAugmentationTransform(
            contrast_range=(0.65, 1.5), p_per_sample=0.15, data_key=target_key
        )
    )
    tr_transforms.append(
        SimulateLowResolutionTransform(
    ...
    tr_transforms.append(SharpeningTransform(p_per_sample=0.2, data_key=target_key))
    tr_transforms.append(
        GammaTransform(
    ...
    if params.get("do_mirror") or params.get("mirror"):
        tr_transforms.append(
            MirrorTransform(
                params.get("mirror_axes"), data_key=target_key, label_key=label_key
            )
        )
    ...
    if label_key is not None:
        tr_transforms.append(
            RemoveLabelTransform(-1, 0, input_key=label_key, output_key=label_key)
        )
        ...
        if params.get("move_last_seg_chanel_to_data") is not None and params.get(
            ...
            tr_transforms.append(
                MoveSegAsOneHotToData(
                    1, params.get("all_segmentation_labels"), label_key, target_key
                )
            )
