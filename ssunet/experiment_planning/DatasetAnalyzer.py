...
from ssunet.paths import nnUNet_raw_data, nnUNet_cropped_data
import numpy as np
import pickle
from ssunet.preprocessing.cropping import get_patient_identifiers_from_cropped_files
...
class DatasetAnalyzer(object):
    ...
    def get_classes(self):
        ...
        return datasetjson['labels'] if 'labels' in datasetjson.keys() else None
    ...
    def analyze_dataset(self, collect_intensityproperties=True):
        ...
        all_classes = [int(i) for i in classes.keys() if int(i) > 0] if classes is not None else None
