...
def copy_fold(in_folder: str, out_folder: str):
    ...
    shutil.copy(
        join(in_folder, "model_final_checkpoint.model"),
        join(out_folder, "model_final_checkpoint.model"),
    )
    shutil.copy(
        join(in_folder, "model_final_checkpoint.model.pkl"),
    ...
    if isfile(join(in_folder, "network_architecture.pdf")):
        shutil.copy(
            join(in_folder, "network_architecture.pdf"),
...
def copy_model(directory: str, output_directory: str):
    ...
    shutil.copy(
        join(directory, "postprocessing.json"),
        join(output_directory, "postprocessing.json"),
    )


def copy_pretrained_models_for_task(
    task_name: str,
    ...
    for m in models:
        to = (
            trainer_output_dir_cascade
            if m == "3d_cascade_fullres"
            else trainer_output_dir
        )
...
def check_if_valid(ensemble: str, valid_models, valid_trainers, valid_plans):
    ...
    if c1 not in valid_models:
        return False
    if c2 not in valid_models:
        return False
    if tr1 not in valid_trainers:
        return False
    if tr2 not in valid_trainers:
        return False
    if p1 not in valid_plans:
        return False
    if p2 not in valid_plans:
        return False
    return True
...
def copy_ensembles(
    taskname,
    output_folder,
    valid_models=("2d", "3d_fullres", "3d_lowres", "3d_cascade_fullres"),
...
def export_one_task(
    ...
    copy_pretrained_models_for_task(
        taskname,
        output_folder,
        models,
        nnunet_trainer,
        nnunet_trainer_cascade,
        plans_identifier,
    )
    copy_ensembles(
        taskname,
    ...
    compress_folder(
        join(output_folder, taskname + ".zip"), join(output_folder, taskname)
    )
...
def export_pretrained_model(
    ...
    for m in models:
        ...
        ), ("not all requested folds " "present, " "Task %s model %s" % (task_name, m))

        assert isfile(
            join(expected_output_folder, "plans.pkl")
        ), "plans.pkl missing, Task %s model %s" % (task_name, m)

        for e in expected_folders:
            zipf.write(
                join(expected_output_folder, e, "debug.json"),
                os.path.relpath(
                    join(expected_output_folder, e, "debug.json"),
                    network_training_output_dir,
                ),
            ...
            zipf.write(
                join(expected_output_folder, e, "model_final_checkpoint.model"),
                os.path.relpath(
                    join(expected_output_folder, e, "model_final_checkpoint.model"),
                    network_training_output_dir,
                ),
            ...
            zipf.write(
                join(expected_output_folder, e, "model_final_checkpoint.model.pkl"),
                os.path.relpath(
                    join(expected_output_folder, e, "model_final_checkpoint.model.pkl"),
                    network_training_output_dir,
                ),
            ...
            zipf.write(
                join(expected_output_folder, e, "progress.png"),
                os.path.relpath(
                    join(expected_output_folder, e, "progress.png"),
                    network_training_output_dir,
                ),
            ...
            if isfile(join(expected_output_folder, e, "network_architecture.pdf")):
                zipf.write(
                    join(expected_output_folder, e, "network_architecture.pdf"),
                    os.path.relpath(
                        join(expected_output_folder, e, "network_architecture.pdf"),
                        network_training_output_dir,
                    ),
        ...
        zipf.write(
            join(expected_output_folder, "plans.pkl"),
            os.path.relpath(
                join(expected_output_folder, "plans.pkl"), network_training_output_dir
            ),
        )
        if not isfile(join(expected_output_folder, "postprocessing.json")):
            if strict:
                raise RuntimeError(
                    "postprocessing.json missing. Run nnUNet_determine_postprocessing or disable strict"
                )
            else:
                print("WARNING: postprocessing.json missing")
        else:
            zipf.write(
                join(expected_output_folder, "postprocessing.json"),
                os.path.relpath(
                    join(expected_output_folder, "postprocessing.json"),
                    network_training_output_dir,
                ),
    ...
    for s in subd:
        v = check_if_valid(
            s, models, (nnunet_trainer, nnunet_trainer_cascade), (plans_identifier)
        )
        if v:
            valid.append(s)
    for v in valid:
        zipf.write(
            join(ensemble_dir, v, "postprocessing.json"),
            os.path.relpath(
                join(ensemble_dir, v, "postprocessing.json"),
                network_training_output_dir,
            ),
...
def export_entry_point():
    ...
    parser = argparse.ArgumentParser(
        description="Use this script to export models to a zip file for sharing with "
    ...
    parser.add_argument(
        "-pl",
        type=str,
        help="nnunet plans identifier. Default: %s" % default_plans_identifier,
        required=False,
    ...
    parser.add_argument(
        "--disable_strict",
        action="store_true",
        help="set this if you want to allow skipping " "missing things",
        required=False,
    )
    parser.add_argument(
        "-f",
